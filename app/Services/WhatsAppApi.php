...
/**
 * WhatsApp API Client for UltraMsg
 */
class WhatsAppApi
{
    ...
    public function __construct(string $token, string $instance_id)
    {
        $this->token = $token;
        $this->instance_id = 'instance'.preg_replace('/[^0-9]/', '', $instance_id);
        $this->baseUrl = rtrim(config('ultramsg.base_url', 'https://api.ultramsg.com'), '/');
    }

    // messages
    public function getMessages($page = 1, $limit = 100, $status = 'all', $sort = 'asc', $id = '', $referenceId = '', $from = '', $to = '', $ack = '')
    {
        $params = ['page' => $page, 'limit' => $limit, 'status' => $status, 'sort' => $sort, 'id' => $id, 'referenceId' => $referenceId, 'from' => $from, 'to' => $to, 'ack' => $ack];
    ...
    {
        $params = ['to' => $to, 'body' => $body, 'priority' => $priority, 'referenceId' => $referenceId];
    ...
    {
        $params = ['to' => $to, 'caption' => $caption, 'image' => $image, 'priority' => $priority, 'referenceId' => $referenceId, 'nocache' => $nocache];
    ...
    {
        $params = ['to' => $to, 'sticker' => $sticker, 'priority' => $priority, 'referenceId' => $referenceId, 'nocache' => $nocache];
    ...
    {
        $params = ['to' => $to, 'filename' => $filename, 'document' => $document, 'caption' => $caption, 'priority' => $priority, 'referenceId' => $referenceId, 'nocache' => $nocache];
    ...
    {
        $params = ['to' => $to, 'audio' => $audio, 'priority' => $priority, 'referenceId' => $referenceId, 'nocache' => $nocache];
    ...
    {
        $params = ['to' => $to, 'caption' => $caption, 'video' => $video, 'priority' => $priority, 'referenceId' => $referenceId, 'nocache' => $nocache];
    ...
    {
        $params = ['to' => $to, 'link' => $link, 'priority' => $priority, 'referenceId' => $referenceId];
    ...
    {
        $params = ['to' => $to, 'contact' => $contact, 'priority' => $priority, 'referenceId' => $referenceId];
    ...
    {
        $params = ['to' => $to, 'address' => $address, 'lat' => $lat, 'lng' => $lng, 'priority' => $priority, 'referenceId' => $referenceId];
    ...
    {
        $params = ['to' => $to, 'vcard' => $vcard, 'priority' => $priority, 'referenceId' => $referenceId];
    ...
    public function sendInstanceSettings($sendDelay = '1', $webhook_url = '', $webhook_message_received = false, $webhook_message_create = false, $webhook_message_ack = false, $webhook_message_download_media = false)
    {
        $params = [
            ...
            'webhook_message_received' => json_encode($webhook_message_received),
            'webhook_message_create' => json_encode($webhook_message_create),
            'webhook_message_ack' => json_encode($webhook_message_ack),
    ...
    public function sendRequest($method, $path, $params = [])
    {
        ...
        $url = $this->baseUrl.'/'.$this->instance_id.'/'.$path;
